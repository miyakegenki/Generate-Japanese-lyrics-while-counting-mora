...
# 正しいモデル名（2025年時点）
MODEL_NAME = "gemini-1.5-flash"  # または "gemini-1.5-flash-001", "gemini-1.5-pro"
MAX_ITERS = 10
LOG_BASE_DIR = "logs"
    ...
    def send(self, prompt: str, tag: str) -> str:
        ...
        try:
            ...
            if "429" in str(e) or "quota" in str(e).lower():
                raise RuntimeError(
                    "Google Gemini API の無料枠（クォータ）を使い切りました。\n"
                    "原因:\n"
                    "  - 今日の無料リクエスト数の上限に達した\n"
                    "  - 1分あたりのリクエスト数制限に達した\n"
                    "対処法:\n"
                    "  1. 30秒〜数時間待ってから再実行\n"
                    "  2. 翌日まで待つ（日次制限の場合）\n"
                    "  3. より軽量なモデル（flash-lite）を使う\n"
                    "  4. Google AI Studio で有料プランに切り替える\n"
                    f"\n元のエラー: {e}"
                )
            raise
    ...
    try:
        ...
        for model in genai.list_models():
            if 'generateContent' in model.supported_generation_methods:
                ...
                print(f"  見つかりました: {model.name}")
        
        if not available_models:
            print("エラー: generateContentをサポートするモデルが見つかりません")
            return None
        
        # 優先順位: 軽量で無料枠に優しいFlash系を優先
        # Pro系は重く、無料枠を早く消費するため最後
        priority = [
            # 最新Flash系（軽量・高速・無料枠に優しい）
        ...
        for preferred in priority:
            if preferred in available_models:
                print(f"\n✓ 選択されたモデル: {preferred}")
                print("  （軽量モデルを優先して選択しました）")
                return preferred
        
        # 優先順位になければFlashまたはLiteを含むものを探す
        for model in available_models:
            if 'flash' in model.lower() or 'lite' in model.lower():
                print(f"\n✓ 選択されたモデル: {model}")
                print("  （Flash/Lite系モデルを選択しました）")
                return model
        
        # どうしても見つからなければ最初のもの
        selected = available_models[0]
        print(f"\n⚠ 選択されたモデル: {selected}")
        print("  （警告: 優先モデルが見つからなかったため、最初のモデルを使用）")
        return selected
...
def generate_lyrics_with_mora_check(theme: str, mood: str, mora_structure: List[int], verses: int, model_name: str) -> Optional[str]:
    if not API_KEY:
...
if __name__ == "__main__":
    ...
    if not model_name:
        ...
        print("APIキーを確認するか、https://ai.google.dev/ でアクセス権を確認してください。")
        exit(1)
    ...
    result = generate_lyrics_with_mora_check(THEME, MOOD, MORA_STRUCTURE, VERSES, model_name)
    if result:
        print("\n" + "="*50)
        print("完成した歌詞:")
        print("="*50)
